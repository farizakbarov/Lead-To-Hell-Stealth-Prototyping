	...
	public class OffScreenIndicatorManagerCanvas : OffScreenIndicatorManager {
		...
		public override void AddIndicator(Transform target, int indicatorID){
			...
			if (!ExistsIndicator(target)){
                ...
                newArrowIndicator.arrow.GetComponent<Image>().color = Color.clear;
                ...
                if(target.GetComponent<LTHMoveAnimator>() != null)
                {
                    target.GetComponent<LTHMoveAnimator>().myIndicator = newArrowIndicator.arrow;
                    //newArrowIndicator.arrow.AddComponent<ArrowStore>();
                    //newArrowIndicator.arrow.GetComponent<ArrowStore>().TrackingObject = target.gameObject;
                }
			} else {
		...
		protected override void UpdateIndicatorPosition(ArrowIndicator arrowIndicator, int id = 0){
            LTHMoveAnimator script = arrowIndicator.target.GetComponent<LTHMoveAnimator>();


            Vector3 v2DPos = Camera.main.WorldToScreenPoint(arrowIndicator.target.localPosition + arrowIndicator.indicator.targetOffset + script.IndicatorOffset);
			float angle;
			bool behindCamera;
